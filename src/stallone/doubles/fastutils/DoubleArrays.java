...
public class DoubleArrays
{
    ...
    private static void insertionSort(final double[] a, final int from, final int to)
    {
        for (int i = from; ++i < to;)
        {
            double t = a[ i];
            int j = i;
            for (double u = a[ j - 1]; ((t) < (u)); u = a[ --j - 1])
            {
                a[ j] = u;
                if (from == j - 1)
                {
    ...
    public static void radixSort(final double[] a, final int from, final int to)
    {
        ...
        while (offsetPos > 0)
        {
            ...
            {
                digit[ i] = (byte) (((fixDouble(a[ first + i]) >>> shift) & DIGIT_MASK) ^ signMask);
            }
            for (int i = length; i-- != 0;)
            {
                count[ digit[ i] & 0xFF]++;
    ...
    public static void radixSortIndirect(final int[] perm, final double[] a, final int from, final int to, final boolean stable)
    {
        ...
        while (offsetPos > 0)
        {
            ...
            for (int i = length; i-- != 0;)
            {
                digit[ i] = (byte) (((fixDouble(a[ perm[ first + i]]) >>> shift) & DIGIT_MASK) ^ signMask);
            }
            for (int i = length; i-- != 0;)
            {
                count[ digit[ i] & 0xFF]++;
            ...
            if (stable)
            {
                for (int i = length; i-- != 0;)
                {
                    support[ --count[ digit[ i] & 0xFF]] = perm[ first + i];
                }
    ...
    public static void radixSort(final double[] a, final double[] b, final int from, final int to)
    {
        ...
        while (offsetPos > 0)
        {
            ...
            for (int i = length; i-- != 0;)
            {
                digit[ i] = (byte) (((fixDouble(k[ first + i]) >>> shift) & DIGIT_MASK) ^ signMask);
            }
            for (int i = length; i-- != 0;)
            {
                count[ digit[ i] & 0xFF]++;
    ...
    public static void radixSortIndirect(final int[] perm, final double[] a, final double[] b, final int from, final int to, final boolean stable)
    {
        ...
        while (offsetPos > 0)
        {
            ...
            for (int i = length; i-- != 0;)
            {
                digit[ i] = (byte) (((fixDouble(k[ perm[ first + i]]) >>> shift) & DIGIT_MASK) ^ signMask);
            }
            for (int i = length; i-- != 0;)
            {
                count[ digit[ i] & 0xFF]++;
