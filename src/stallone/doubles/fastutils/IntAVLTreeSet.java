 ...
 */
public class IntAVLTreeSet extends AbstractIntSortedSet implements java.io.Serializable, Cloneable, IntSortedSet {
 ...
 private class SetIterator extends AbstractIntListIterator {
  ...
  public void remove() {
   ...
   updatePrevious();
   updateNext();
   IntAVLTreeSet.this.remove( curr.key );
   curr = null;
  }
 }
 ...
 private final class Subset extends AbstractIntSortedSet implements java.io.Serializable, IntSortedSet {
  ...
  public Subset( final int from, final boolean bottom, final int to, final boolean top ) {
   if ( ! bottom && ! top && IntAVLTreeSet.this.compare( from, to ) > 0 ) throw new IllegalArgumentException( "Start element (" + from + ") is larger than end element (" + to + ")" );
   this.from = from;
   this.bottom = bottom;
   this.to = to;
   this.top = top;
  ...
  final boolean in( final int k ) {
   return ( bottom || IntAVLTreeSet.this.compare( k, from ) >= 0 ) &&
  ...
  public int firstInt() {
   IntAVLTreeSet.Entry e = firstEntry();
   if ( e == null ) throw new NoSuchElementException();
   return e.key;
  }
  public int lastInt() {
   IntAVLTreeSet.Entry e = lastEntry();
   if ( e == null ) throw new NoSuchElementException();
   return e.key;
  }
  ...
  private final class SubsetIterator extends SetIterator {
   ...
   void updatePrevious() {
    prev = prev.prev();
    if ( ! bottom && prev != null && IntAVLTreeSet.this.compare( prev.key, from ) < 0 ) prev = null;
   }
   void updateNext() {
    next = next.next();
    if ( ! top && next != null && IntAVLTreeSet.this.compare( next.key, to ) >= 0 ) next = null;
   }
  }
 }
