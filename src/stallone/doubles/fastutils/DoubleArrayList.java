...
public class DoubleArrayList extends AbstractDoubleList implements RandomAccess, Cloneable, java.io.Serializable {
 ...
 public void size( final int size ) {
  ...
  if ( size > this.size ) DoubleArrays.fill( a, this.size, size, (0) );
  this.size = size;
 }
 ...
 public DoubleListIterator listIterator( final int index ) {
  ...
  return new AbstractDoubleListIterator () {
    ...
    public double nextDouble() { if ( ! hasNext() ) throw new NoSuchElementException(); return a[ last = pos++ ]; }
    public double previousDouble() { if ( ! hasPrevious() ) throw new NoSuchElementException(); return a[ last = --pos ]; }
